    ...
    try:
        with open(TEST_API_FILE, "r", encoding="utf-8") as f:
            ...
            if not isinstance(data, list):
                logger.error(
                    f"В файле {TEST_API_FILE} данные не являются списком, а {type(data)}"
                )
                return None
            logger.debug(
                f"Данные загружены из {TEST_API_FILE}. Найдено активов: {len(data)}"
            )
            return data
        ...
        logger.error(
            f"Ошибка при чтении JSON файла {TEST_API_FILE}: {e}", exc_info=True
        )
        return None
...
async def fetch_assets():
    ...
    if TEST_API:
        ...
        if data is None:
            ...
            return None, None
        logger.info(
            f"Тестовый режим: данные загружены из файла. Найдено активов: {len(data)}"
        )
        return data, None
    ...
    try:
        ...
        if proxy_url:
            logger.info(
                f"Использование прокси: {proxy_url.split('@')[1] if '@' in proxy_url else proxy_url}"
            )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                API_URL, proxy=proxy_url, headers=headers
            ) as response:
                if response.status == 200:
                    ...
                    if not isinstance(data, list):
                        ...
                        return None, None
                    logger.info(
                        f"Данные успешно получены с API. Найдено активов: {len(data)}"
                    )
                    return data, None
                else:
                    logger.warning(
                        f"Ошибка при получении данных с API. Статус: {response.status}"
                    )
...
async def load_assets_from_json():
    ...
    try:
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            ...
            if not isinstance(data, list):
                logger.error(
                    f"В файле {DATA_FILE} данные не являются списком, а {type(data)}"
                )
                return None
            logger.debug(
                f"Данные загружены из {DATA_FILE}. Найдено активов: {len(data)}"
            )
            return data
    except FileNotFoundError:
...
async def cmd_demo(message: types.Message):
    ...
    demo_notifications = [
        ...
        {
            "type": "epoch_changed",
            "message": f'🆕✨🔄 <b>NEW EPOCH!</b> ✨🆕\n\n<b>{demo_asset_name}</b>\nEpoch: {demo_old_epoch} → {demo_new_epoch}\nFilled: {demo_tvl:,} / {demo_cap:,}\n\n<a href="https://app.piggybank.fi/">Open PiggyBank</a>\n\n<i>⚠️ This is a demo notification</i>',
        },
        {
        ...
        },
        {
            ...
            "message": f'⚡️🔧⚡️ <b>CAPACITY LIMIT CHANGED</b> ⚡️🔧⚡️\n\n<b>{demo_asset_name}</b> ({demo_ticker})\nChange: +{demo_cap_change_positive:.2f}\nFilled: {demo_tvl:,} / {demo_cap:,}\n\n<a href="https://app.piggybank.fi/">Open PiggyBank</a>\n\n<i>⚠️ This is a demo notification</i>',
        },
    ]
    ...
    for notification in demo_notifications:
        try:
            # Добавляем текст в конец каждого демо-уведомления
            message_with_footer = f"{notification['message']}\n\nSay /thankyou 😊"
            await message.answer(
                message_with_footer,
                parse_mode="HTML",
            ...
            logger.error(
                f"Ошибка при отправке демо-уведомления {notification['type']}: {e}",
                exc_info=True,
            )
...
async def cmd_get_data(message: types.Message):
    ...
    try:
        ...
        processing_msg = await message.answer(
            "⏳ Generating data export...", parse_mode="HTML"
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for table in tables:
                ...
                try:
                    ...
                    logger.error(
                        f"Ошибка при экспорте таблицы {table}: {e}", exc_info=True
                    )
                    await message.answer(
                        f"❌ Error exporting table {table}: {e}", parse_mode="HTML"
                    )
...
🏆 <b>Top 5 Assets:</b>"""
            ...
            for csv_path, filename in csv_files:
                try:
                    ...
                    logger.error(
                        f"Ошибка при отправке CSV файла {filename}: {e}", exc_info=True
                    )
                    await message.answer(
                        f"❌ Error sending {filename}: {e}", parse_mode="HTML"
                    )
            ...
            if os.path.exists(LOG_FILE) and os.path.isfile(LOG_FILE):
                try:
                    ...
                    await message.answer(
                        f"❌ Error sending log file: {e}", parse_mode="HTML"
                    )
            else:
                await message.answer(
                    "⚠️ Log file not found or is a directory", parse_mode="HTML"
                )
...
For all questions and suggestions, please contact maxphoton93@gmail.com or discord @maxphoton. Also <a href="https://x.com/maxproton_astra">my X</a>"""
...
@dp.message(Command("thankyou"))
async def cmd_thankyou(message: types.Message):
    """Обработчик команды /thankyou - отправка изображения с благодарностью"""
    user = message.from_user
    if not user:
        return

    logger.info(f"Команда /thankyou от пользователя {user.id} (@{user.username})")

    try:
        # Проверяем существование файла
        image_path = "oinks.png"
        if not os.path.exists(image_path):
            logger.error(f"Файл {image_path} не найден")
            await message.answer(
                "❌ Image file not found. Please contact the administrator.",
                parse_mode="HTML",
            )
            return
        ...
        # Текст подписи на английском
        caption = """Thank you too! 
        
You can show your appreciation by using my referral code <code>F1TK565JOT</code> on the page <a href="https://app.piggybank.fi/oinks">https://app.piggybank.fi/oinks</a>. 
        
It will give you a 5% oink boost 🫶"""
        ...
        logger.info(f"Изображение с благодарностью отправлено пользователю {user.id}")

    except Exception as e:
        logger.error(
            f"Ошибка при отправке изображения пользователю {user.id}: {e}",
            exc_info=True,
        )
        await message.answer(
            "❌ Error sending image. Please try again later.", parse_mode="HTML"
        )
...
async def cmd_get_stats(message: types.Message):
    ...
    try:
        ...
        if assets_data is None:
            logger.warning(
                f"Не удалось получить данные с API для пользователя {user.id}"
            )
            await message.answer(
                "❌ Failed to fetch data from API. Please try again later.",
                parse_mode="HTML",
...
async def process_asset_toggle(callback: types.CallbackQuery):
    ...
    if assets_data is None:
        logger.warning(
            f"Не удалось загрузить данные для переключения подписки пользователя {user.id}"
        )
    ...
    asset = next(
        (a for a in assets_data if a.get("asset_ticker") == asset_ticker), None
    )
    if not asset:
    ...
    if is_subscribed:
        logger.info(
            f"Пользователь {user.id} подписался на {asset_name} ({asset_ticker})"
        )
        await callback.answer(f"✅ Notifications for {asset_name} enabled")
    else:
        logger.info(
            f"Пользователь {user.id} отписался от {asset_name} ({asset_ticker})"
        )
    ...
    try:
        ...
        logger.warning(
            f"Не удалось обновить клавиатуру для пользователя {user.id}: {e}"
        )
...
Click on an asset to enable/disable notifications"""
        await callback.message.answer(
            text, reply_markup=new_keyboard, parse_mode="HTML"
        )
...
async def check_assets_changes():
    ...
    logger.debug(
        f"Сравнение: сохранено {len(saved_dict)} активов, текущих {len(current_dict)} активов"
    )
    ...
    for ticker, current_asset in current_dict.items():
        ...
        saved_has_epoch = (
            saved_asset and "epoch" in saved_asset if saved_asset else False
        )
        ...
        if current_has_epoch and not saved_has_epoch:
            ...
            logger.info(
                f"Обнаружено появление epoch для актива {asset_name} ({ticker}). Пользователей для уведомления: {len(all_users)}"
            )
    ...
    for ticker, current_asset in current_dict.items():
        ...
        if saved_asset and "epoch" in current_asset and "epoch" in saved_asset:
            ...
            if current_epoch != saved_epoch:
                ...
                if subscribed_users:
                    logger.info(
                        f"Обнаружено изменение epoch для {asset_name} ({ticker}): {saved_epoch} → {current_epoch}. Подписчиков: {len(subscribed_users)}"
                    )
                    ...
                    notifications.append(
                        {
                            ...
                            "message": f'🆕✨🔄 <b>NEW EPOCH!</b> ✨🆕\n\n<b>{asset_name}</b>\nEpoch: {saved_epoch} → {current_epoch}{capacity_info}\n\n<a href="https://app.piggybank.fi/">Open PiggyBank</a>',
                        }
                    )
            ...
            if current_lst_tvl is not None and saved_lst_tvl is not None:
                try:
                    ...
                    if change_abs > 1.0:
                        ...
                        logger.info(
                            f"Изменение lst_tvl для {asset_name} ({ticker}): {saved_tvl_float} → {current_tvl_float} ({change_sign}{change})"
                        )
                        ...
                        if subscribed_users:
                            ...
                            logger.info(
                                f"Уведомление об изменении lst_tvl для {asset_name} ({ticker}) добавлено в очередь. Подписчиков: {len(subscribed_users)}"
                            )
                except (ValueError, TypeError) as e:
                    logger.warning(
                        f"Ошибка при преобразовании значений lst_tvl для {ticker}: {e}"
                    )
                    continue
            ...
            if current_lst_cap is not None and saved_lst_cap is not None:
                try:
                    ...
                    if change_abs > 0:
                        ...
                        change_sign = "+" if change > 0 else ""
                        logger.info(
                            f"Изменение lst_cap для {asset_name} ({ticker}): {saved_cap_float} → {current_cap_float} ({change_sign}{change})"
                        )
                        ...
                        if subscribed_users:
                            ...
                            user_message = f'⚡️🔧⚡️ <b>CAPACITY LIMIT CHANGED</b> ⚡️🔧⚡️\n\n<b>{asset_name}</b> ({ticker})\nChange: {change_text}{capacity_info}\n\n<a href="https://app.piggybank.fi/">Open PiggyBank</a>'
                            notifications.append(
                                {
                            ...
                            logger.info(
                                f"Уведомление об изменении lst_cap для {asset_name} ({ticker}) добавлено в очередь. Подписчиков: {len(subscribed_users)}"
                            )
                except (ValueError, TypeError) as e:
                    logger.warning(
                        f"Ошибка при преобразовании значений lst_cap для {ticker}: {e}"
                    )
                    continue
...
async def send_notifications(notifications):
    ...
    for notification in notifications:
        ...
        logger.info(
            f"Рассылка уведомления типа '{notification_type}' для актива {asset_name}. Получателей: {len(users)}"
        )

        # Добавляем текст в конец каждого уведомления
        message_with_footer = f"{message_text}\n\nSay /thankyou 😊"
...
async def notify_admin_about_api_error(error_status, error_message=None):
    ...
    try:
        ...
        logger.error(
            f"Не удалось отправить уведомление админу об ошибке API: {e}", exc_info=True
        )
        ...
        try:
            ...
            if error_status is not None:
                ...
                if error_status in [429, 503]:
                    logger.warning(
                        f"Получена ошибка API {error_status}. Следующая проверка через {error_wait_interval} секунд (5 минут)"
                    )
                ...
                logger.info(
                    "Запрос к API успешен. Возвращаемся к обычному интервалу проверки (60 секунд)"
                )
                wait_interval = 60
...
async def main():
    ...
    if not BOT_TOKEN or BOT_TOKEN == "":
        logger.error(
            "BOT_TOKEN не установлен! Установите токен в переменной окружения BOT_TOKEN"
        )
        return
    ...
    if ADMIN_ID:
        try:
            await bot.send_message(
                ADMIN_ID, "✅ Bot has been started successfully!", parse_mode="HTML"
            )
