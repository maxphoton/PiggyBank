...
async def toggle_subscription(user_id: int, asset_ticker: str, asset_name: str = None):
    ...
    try:
        ...
        logger.error(f"Ошибка при переключении подписки пользователя {user_id} на {asset_ticker}: {e}", exc_info=True)
        raise
...
async def get_user_subscriptions(user_id: int):
    ...
    try:
        ...
        logger.error(f"Ошибка при получении подписок пользователя {user_id}: {e}", exc_info=True)
...
async def get_subscribed_users(asset_ticker: str):
    ...
    try:
        ...
        logger.error(f"Ошибка при получении подписчиков актива {asset_ticker}: {e}", exc_info=True)
...
async def export_table_to_csv(table_name: str, csv_file_path: str):
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as db:
            ...
            logger.info(f"Таблица {table_name} экспортирована в {csv_file_path}. Записей: {len(rows)}")
            return len(rows)
    except Exception as e:
...
async def get_bot_statistics():
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as db:
            ...
            cursor = await db.execute('SELECT COUNT(DISTINCT asset_ticker) FROM user_subscriptions')
            unique_assets = (await cursor.fetchone())[0]
            
            # Пользователи с подписками
            cursor = await db.execute('SELECT COUNT(DISTINCT user_id) FROM user_subscriptions')
